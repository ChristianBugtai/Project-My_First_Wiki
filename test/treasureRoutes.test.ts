...
describe("Treasure Routes", () => {
    afterEach(() => {
        jest.clearAllMocks();
    });

    describe("GET /api/v1/treasures/:id", () => {
        it("should call the getEntryById controller", async () => {
            const mockId: string = "101";

            await request(app).get(`/api/v1/treasures/${mockId}`)
            expect(getEntryById).toHaveBeenCalled
        })
    })

    describe("GET /api/v1/treasures/", () => {
        it("should call the getEntries controller", async () => {

            await request(app).get(`/api/v1/treasures/}`)
            expect(getEntries).toHaveBeenCalled
        })
    })

    describe("POST /api/v1/treasures/", () => {
        it("should call the addEntry controller", async () => {

            await request(app).post(`/api/v1/treasures/}`)
            expect(addEntry).toHaveBeenCalled
        })
    })

    describe("PUT /api/v1/treasures/:id", () => {
        it("should call the updateEntry controller", async () => {
            const mockId: string = "101";

            await request(app).put(`/api/v1/treasures/${mockId}}`)
            expect(updateEntry).toHaveBeenCalled
        })
    })

    describe("DELETE /api/v1/treasures/:id", () => {
        it("should call the updateEntry controller", async () => {
            const mockId: string = "101";

            await request(app).delete(`/api/v1/treasures/${mockId}}`)
